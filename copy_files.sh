...
# Usage: ./cp_files.sh <original_dir> <new_dir> <file_extension_if_any>
...
do
	...
	printf "\nCopying $FILE to $NEW_DIR/$FILE_NAME"
	cp $FILE $NEW_DIR/$FILE_NAME
	FILE_COUNT=$((FILE_COUNT+1))
done

printf "\n\nFile copy completed. Copied $FILE_COUNT files from $ORG_DIR to $NEW_DIR\n"
exit 0
